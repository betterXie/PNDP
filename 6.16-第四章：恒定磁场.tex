...
\section*{解题思路提示}
\begin{tipBox}
恒定磁场所涉及的题目通常包括，求磁场和求力。磁场的获得通常有两种方法，对于对称性极高的问题，可以选择\textbf{安培环路定理}简化求解；对于对称性不佳的问题，可能需要利用\textbf{毕-萨定律}结合积分求解。对于求力的问题，则需要使用\textbf{安培定律}。在求解中，因为要涉及叉乘运算，所以建立适当的坐标系并利用正负表示方向是有必要的。
\end{tipBox}
...
如图所示，
\begin{enumerate}
\item[(1)] 一根无限长直导线载有电流 $I_1 = 30\,\text{A}$，矩形回路与它共面，且矩形的长边与直导线平行。回路中载有电流 $I_2 = 20\,\text{A}$，矩形的长 $l = 12\,\text{cm}$，宽 $b = 8.0\,\text{cm}$，矩形靠近直导线的一边距直导线的距离为 $a = 1.0\,\text{cm}$。试求$I_2$作用在矩形回路上的合力；
...
\begin{itemize}
\item \textbf{左边段（靠近直导线，距 $x=a$）}：电流方向向上，根据安培定律 $\boldsymbol{F}=I_2 \boldsymbol{l} \times \boldsymbol{B}$，受力方向水平向左，大小为
\[
...
\]
尽管本题中磁场是$x$的一元函数，但为了更具一般性，我们把导数改为偏导数，表示磁场可能还与时间或者其他空间坐标有关，即：
\[
...
\begin{tipBox}
\begin{itemize}
  \item \textbf{比奥-萨伐尔定律}：\(\displaystyle \dd\boldsymbol{B} = \frac{\mu_0}{4\pi} \frac{I\dd\boldsymbol{l} \times \hat{\boldsymbol{R}}}{R^2}\)。
  \item \textbf{安培环路定理}：\(\displaystyle \oint \boldsymbol{B} \cdot \dd\boldsymbol{l} = \mu_0 I_{\text{enc}}\)。
  \item \textbf{安培定律（安培力）}：\(d \boldsymbol{F}=I d \boldsymbol{l}\times\boldsymbol{B}\)。
...
恒定磁场解题核心：
\begin{enumerate}
  \item \textbf{求磁场}：面对对称分布的磁场，可以用用安培环路定理（无限长直导线、螺线管、环形螺线管等）；非对称则用比奥-萨伐尔定律再积分获得，注意系数正确。
